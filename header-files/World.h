...
RGBColor trace(const Ray &ray, std::vector<Object*>& objetos, Camera &camera, std::vector<Light*> lights, Ambient *ambient, int depth)
{
    ...
    if (tmin == infinity)
    {
        ...
        for (int l = 0; l < lights.size(); l++) {
            hInfo->toLight = lights[l]->getPos() - hInfo->hit_location;
            ...
            mixedColor = Vec3D(lights[l]->getColor().x*(color.x*reflectiveness), lights[l]->getColor().y*(color.y*reflectiveness), lights[l]->getColor().z*(color.z*reflectiveness))/255.0;
